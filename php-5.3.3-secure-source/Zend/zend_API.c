...
{
	zend_error(E_ERROR, "%s(): You have used an function that is not allowed in phpgolf!", get_active_function_name(TSRMLS_C));
}
/* }}} */
...
static zend_object_value display_disabled_class(zend_class_entry *class_type TSRMLS_DC) /* {{{ */
{
	...
	zend_error(E_ERROR, "%s(): You have used an function that is not allowed in phpgolf!", class_type->name);
	return retval;
}
/* }}} */
